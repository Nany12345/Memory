...
%--------------------------------------------------------------------------
    ...
    properties
        ...
        UEA        = {};                % Unbounded external archive
        oldUEA     = {};                % Save previous
    end
...
%             if size(obj.result,1)<index&&index~=1
%                 tic;
%                 obj.result(index-1,3) = {obj.oldUEA(NDSort(obj.oldUEA.objs,1) == 1)};
%                 % Accumulate the runtime
%                 obj.runtime = obj.runtime + toc;
%                 obj.UEA = [obj.result{index-1,3},obj.UEA(size(obj.oldUEA,2)+1:end)];
%             end
%             obj.oldUEA = obj.UEA;
%             obj.result(index,:) = {obj.evaluated,Population,obj.UEA};
%             % Invoke obj.outputFcn
%             drawnow();
%             if obj.evaluated >= obj.evaluation
%                 tic;
%                 obj.result(index,3) = {obj.UEA(NDSort(obj.UEA.objs,1) == 1)};
%                 % Accumulate the runtime
%                 obj.runtime = obj.runtime + toc;
%             end
%             obj.outputFcn(obj);
...
%         end
        ...
        %       obj.NotTermination(Population)
            ...
            if obj.save<=0; num=10; else; num=obj.save; end
            index = max(1,min(min(num,size(obj.result,1)+1),ceil(num*obj.evaluated/obj.evaluation)));
            obj.result(index,:) = {obj.evaluated,obj.UEA};
            obj.UEA = [];
            % Invoke obj.outputFcn
            drawnow();
            obj.outputFcn(obj);
