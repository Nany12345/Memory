function [FrontNo,MaxFNo,CountComp,Relation] = NDSortENSNDT_Mem(varargin)
...
%--------------------------------------------------------------------------
    ...
    Relation = varargin{4};
    MaxFNo = 1;
    [FrontNo,CountComp] = ENS_NDT_Mem(PopObj1,PopObj2);
end

function [FrontNo,CountComp] = ENS_NDT_Mem(PopObj1,PopObj2)
    CountComp = 0;
    B = 2;
    PopObj = [PopObj1;PopObj2];
    [PopObj,Loa,Loc] = unique(PopObj(:,end:-1:1),'rows');
    PopObj = PopObj(:,end:-1:1);
    N1 = size(PopObj1,1);
    LoaOld = find(Loa<=N1);
    LoaNew = find(Loa>N1);
    [N,M] = size(PopObj);
    ...
    for i = 2:N
        if CNO <= length(LoaNew) && i == LoaNew(CNO)
            CurNode = 1;
            Dominated = false;
            ...
            CNO = CNO+1;
        elseif N1 > 0 && COO <= length(LoaOld) && i == LoaOld(COO)
            Dominated = false;
            for j = CNO-1:-1:1
                if FrontNo(LoaNew(j)) == 1
                    CountComp = CountComp+1;
                    m = 1;
